...
function imaginaryDownload() {
    sectionDownload(imaginary_subs_1.subs, { limit: 200 });
}
;
function otherDownload() {
    sectionDownload(other_subs_1.subs, { limit: 800 });
}
function dungeonDownload() {
    sectionDownload(dungeon_subs_1.subs, { limit: 50 });
}
...
async function cleanUnwanted() {
    let found = 0;
    let folders = Object.keys(imaginary_subs_1.subs).map(key => imaginary_subs_1.subs[key].folderPath);
